...
public class demoTest {
    ...
    public void sauceTest(String data){
        ...
        Assert.assertEquals(loginURL, "https://www.saucedemo.com/");

        // Login in the system and assert that you are in listing page
        listingPage = loginPage.logIn(user[0], user[1]);
        Assert.assertEquals(listingPage.getUrl(), "https://www.saucedemo.com/inventory.html");

        // add two items in cart and assert that items added successfully
        listingPage.addToCart();
        Assert.assertEquals(listingPage.checkItemOneAdded(), "Remove");
        Assert.assertEquals(listingPage.checkItemTwoAdded(), "Remove");

        // Navigate to cart page and assert that you are in cart page
        cartPage = listingPage.navigateToCart();
        Assert.assertEquals(cartPage.getUrl(), "https://www.saucedemo.com/cart.html");
        ...
        Assert.assertEquals(addressPage.getUrl(), "https://www.saucedemo.com/checkout-step-one.html");

        // create address and assert that you in review order page
        reviewOrderPage = addressPage.createAddress("Mahmoud", "Khaifa", "12345");
        Assert.assertEquals(reviewOrderPage.getUrl(), "https://www.saucedemo.com/checkout-step-two.html");
        ...
        Assert.assertEquals(confirmationOrderPage.getUrl(), "https://www.saucedemo.com/checkout-complete.html");
